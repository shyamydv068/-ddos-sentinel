...
class DatasetPreprocessor:
    ...
    def prepare_data(self) -> tuple[pd.DataFrame, pd.Series, list[str], dict[str, Any]]:
        ...
        if self.label_column not in df.columns:
            ...
            if not label_candidates:
                raise ValueError(
                    "No label column was found. Expected a column named 'label' or similar."
                )
    ...
    def fit_and_transform(self) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.Series, pd.Series, dict[str, Any]]:
